...
local CASE = "planner-chemical-executors"
local failures = {}
local assertions = 0
...
local function finish()
  ...
  local result = {schema=1, case=CASE, status=#failures == 0 and "pass" or "fail",
    ...
    observations={executors=#fixture.executors, completed=storage.completed,
      fixture="Five cycles per selected recipe; declared intermediates, fuel and scripted heat"}}
  ...
  for _, input in ipairs(row.pending) do
    if input.amount > 0 then
      if input.type == "fluid" then
        ...
        input.amount = input.amount - machine.insert{name=input.name, count=input.amount}
      end
    end
  end
end
