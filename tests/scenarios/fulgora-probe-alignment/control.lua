...
local CASE = "fulgora-probe-alignment"
...
script.on_nth_tick(30, function()
  ...
  if storage.stage == "join" then
    ...
    local completed, message = remote.call("nullius-test-bodies", "quick_start", player.index, "fulgora")
    check(completed == nil and message[1] == "nullius-probe.alignment-pending",
      "quick-start must wait for the faction landing")
    check(not player.force.technologies["nullius-probe-fulgora"].researched,
      "pending quick-start changed research")
    check(game.planets["nullius-fulgora"].surface == nil, "pending quick-start created Fulgora")
