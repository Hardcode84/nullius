...
-- place: 46 independent executors at crafting speed 60
...
local cases=require("__nullius-star__/scenarios/module-recipes/fixture")
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("module-test",{width=128,height=96,autoplace_controls={}})
  ...
  for i,case in ipairs(cases) do
    ...
    local other=case.category=="tiny-assembly" and "medium-only-assembly" or "tiny-assembly"
    local position={((i-1)%8)*8-32,math.floor((i-1)/8)*8-20}
    local wrong=surface.create_entity{name="factorio-test-module-"..other,position=position,force="player"}
    wrong.set_recipe(case.name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-module-"..case.category,position=position,force="player"}
...
script.on_nth_tick(600,function(event)
  ...
  helpers.write_file("factorio-tests/module-recipes.json",helpers.table_to_json({
    schema=1,case="module-recipes",status="pass",factorio_version=script.active_mods.base,
