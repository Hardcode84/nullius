...
-- place: 137 independent executors at crafting speed 60
...
local cases=require("__nullius-star__/scenarios/building-recipes/fixture")
...
local renamed={
  ["nullius-small-chest-1"]="wooden-chest",
  ["nullius-small-chest-2"]="iron-chest",
  ["nullius-small-chest-3"]="steel-chest",
  ["nullius-small-storage-chest-2"]="storage-chest",
  ["nullius-small-supply-chest-2"]="passive-provider-chest",
  ["nullius-small-demand-chest-2"]="requester-chest",
  ["nullius-small-buffer-chest-2"]="buffer-chest",
  ["nullius-small-dispatch-chest-2"]="active-provider-chest",
  ["nullius-power-pole-1"]="small-electric-pole",
  ["nullius-power-pole-2"]="medium-electric-pole",
  ["nullius-pylon-1"]="big-electric-pole",
  ["nullius-substation-1"]="substation",
  ["nullius-logic-circuit"]="decider-combinator",
  ["nullius-arithmetic-circuit"]="arithmetic-combinator",
  ["nullius-memory-circuit"]="constant-combinator",
  ["nullius-display-panel"]="display-panel",
  ["nullius-selector-circuit"]="selector-combinator",
  ["nullius-antenna"]="programmable-speaker",
  ["nullius-sensor-node-1"]="radar",
  ["nullius-lamp-1a"]="small-lamp",
  ["nullius-landing-pad"]="cargo-landing-pad",
}
local assertions=0
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("building-test",{width=128,height=512,autoplace_controls={}})
  ...
  for i,case in ipairs(cases) do
    ...
    if script.active_mods["alien-biomes"] and renamed[name] then
      check(prototypes.recipe[name]==nil,name.." original recipe removed")
      name=renamed[name]
    end
    ...
    local other="factorio-test-building-rejected"
    local position={((i-1)%5)*16-32,math.floor((i-1)/5)*16-208}
    local wrong=surface.create_entity{name="factorio-test-building-"..other,position=position,force="player"}
    wrong.set_recipe(name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-building-"..case.category,position=position,force="player"}
    machine.set_recipe(name)
    check(machine.get_recipe().name==name,case.name.." selected")
    storage.feeds[i]={}
    for _,part in ipairs(case.ingredients) do
      if part.type=="item" then
        ...
        local buffer=surface.create_entity{name="factorio-test-building-buffer",position={x,machine.position.y-6},force="player"}
...
script.on_nth_tick(960,function(event)
  ...
  helpers.write_file("factorio-tests/building-recipes.json",helpers.table_to_json({
    schema=1,case="building-recipes",status="pass",factorio_version=script.active_mods.base,
