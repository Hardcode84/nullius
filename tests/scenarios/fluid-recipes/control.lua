...
-- place: 223 independent executors at crafting speed 60
...
local cases=require("__nullius-star__/scenarios/fluid-recipes/fixture")
...
local renamed={
["nullius-rocket-fuel"]="rocket-fuel",
["nullius-stone-recovery"]="stone",
["nullius-methanol-canister-1"]="processed-fuel",
["nullius-desalination"]="nullius-water",
["nullius-carbon-dioxide-to-monoxide"]="nullius-carbon-monoxide",
["nullius-carbon-dioxide-to-methane"]="nullius-methane",
["nullius-methane-to-ethylene"]="nullius-ethylene",
["nullius-salt-electrolysis"]="nullius-sodium",
["nullius-air-filtration"]="nullius-air",
}
local assertions=0
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("fluid-test",{width=128,height=800,autoplace_controls={}})
  surface.request_to_generate_chunks({0,0},13)
  ...
  for i,case in ipairs(cases) do
    ...
    local other="factorio-test-fluid-rejected"
    local position={((i-1)%5)*16-32,math.floor((i-1)/5)*16-352}
    local wrong=surface.create_entity{name="factorio-test-fluid-"..other,position=position,force="player"}
    wrong.set_recipe(name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-fluid-"..case.name,position=position,force="player"}
    ...
    for _,part in ipairs(case.ingredients) do
      if part.type=="item" then
        ...
        local pump=surface.create_entity{name="factorio-test-fluid-pump",position={x,machine.position.y-4.5},direction=defines.direction.south,force="player"}
        local buffer=surface.create_entity{name="factorio-test-fluid-buffer",position={x,machine.position.y-6},force="player"}
...
script.on_nth_tick(960,function(event)
  ...
  helpers.write_file("factorio-tests/fluid-recipes.json",helpers.table_to_json({
    schema=1,case="fluid-recipes",status="pass",factorio_version=script.active_mods.base,
    tick=game.tick,assertions=assertions,recipes=#cases,crafts=#cases+(modern and 6 or 3),failure_count=0,failures={}
  }),false)
end)
