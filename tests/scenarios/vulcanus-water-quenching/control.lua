local CASE = "vulcanus-water-quenching"
local BLOOM = "nullius-molten-iron-bloom"
local PLATE = "nullius-iron-plate"
...
local function finish()
  local result = {schema = 1, case = CASE, tick = game.tick,
...
script.on_nth_tick(1, function()
  ...
  check(not force.recipes["nullius-quenched-iron-plate"].enabled, "Quenching unlocked with dry casting")
  research(force.technologies["nullius-water-quenching"], {})
  check(force.recipes["nullius-quenched-iron-plate"].enabled, "Quenching research did not unlock recipe")
  check(not force.recipes["nullius-boxed-quenched-iron-plate"].enabled, "Bulk quenching unlocked before mass production")
  ...
  check(force.recipes["nullius-boxed-quenched-iron-plate"].enabled, "Mass production did not unlock bulk quenching")
  local dry = force.recipes["nullius-hot-iron-plate"]
  local wet = force.recipes["nullius-quenched-iron-plate"]
  check(dry.energy == wet.energy and wet.energy == 3, "Dry and wet cycle times differ")
  storage.dry = foundry(0, dry.name, 4, 0)
  storage.wet = foundry(12, wet.name, 4, 2)
  storage.bulk = foundry(24, "nullius-boxed-quenched-iron-plate", 20, 10)
...
script.on_nth_tick(240, function()
  ...
  check(storage.dry.machine.get_output_inventory().get_item_count(PLATE) == 3, "Wrong dry yield")
  check(storage.wet.machine.get_output_inventory().get_item_count(PLATE) == 4, "Wrong quenched yield")
...
script.on_nth_tick(2100, function()
  if game.tick == 0 then return end
  script.on_nth_tick(2100, nil)
  ...
  observations.outage.recovered_ingots = storage.sink.get_item_count("nullius-iron-ingot")
  check(storage.outage.machine.products_finished == 0, "Dry outage produced quenched plates")
  check(observations.outage.recovered_ingots == 4, "Inserter did not remove four cooled blooms")
  ...
  check(storage.bulk.machine.get_output_inventory().get_item_count("nullius-box-iron-plate") == 4,
    "Bulk recipe did not produce four boxes")
...
script.on_nth_tick(3000, function()
  if game.tick == 0 then return end
  script.on_nth_tick(3000, nil)
  observations.restart = snapshot(storage.outage)
  observations.restart.plates = storage.sink.get_item_count(PLATE)
  observations.restart.ingots = storage.sink.get_item_count("nullius-iron-ingot")
  check(storage.outage.machine.products_finished == 1, "Foundry did not restart after water returned")
  check(observations.restart.plates == 4, "Restart plates did not reach the output chest")
  check(observations.restart.ingots == 4, "Recovery lost cooled ingots")
