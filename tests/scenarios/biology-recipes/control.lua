...
-- place: 152 independent executors at crafting speed 60
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("biology-test",{width=128,height=576,autoplace_controls={}})
  ...
  for i,case in ipairs(cases) do
    ...
    local other="factorio-test-biology-rejected"
    local position={((i-1)%5)*16-32,math.floor((i-1)/5)*16-240}
    local wrong=surface.create_entity{name="factorio-test-biology-"..other,position=position,force="player"}
    wrong.set_recipe(name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-biology-"..case.category.."-"..inputs.."-"..outputs,position=position,force="player"}
    ...
    for _,part in ipairs(case.ingredients) do
      if part.type=="item" then
        ...
        local pump=surface.create_entity{name="factorio-test-biology-pump",position={x,machine.position.y-4.5},direction=defines.direction.south,force="player"}
        local buffer=surface.create_entity{name="factorio-test-biology-buffer",position={x,machine.position.y-6},force="player"}
...
script.on_nth_tick(960,function(event)
  ...
  helpers.write_file("factorio-tests/biology-recipes.json",helpers.table_to_json({
    schema=1,case="biology-recipes",status="pass",factorio_version=script.active_mods.base,
