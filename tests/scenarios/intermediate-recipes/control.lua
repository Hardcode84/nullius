...
-- place: 293 independent executors at crafting speed 60
...
-- run: 960 ticks
-- expect: exact contracts, one complete batch per recipe, and no input left
local cases=require("__nullius-star__/scenarios/intermediate-recipes/fixture")
...
local renamed={
["nullius-insulated-wire-1"]="copper-cable",
["nullius-concrete-1"]="concrete",
["nullius-reinforced-concrete"]="refined-concrete",
["nullius-hazard-concrete"]="refined-hazard-concrete",
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("intermediate-test",{width=128,height=1024,autoplace_controls={}})
  ...
  for i,case in ipairs(cases) do
    ...
    local other="factorio-test-intermediate-rejected"
    local position={((i-1)%5)*16-32,math.floor((i-1)/5)*16-464}
    local wrong=surface.create_entity{name="factorio-test-intermediate-"..other,position=position,force="player"}
    wrong.set_recipe(name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-intermediate-"..case.name,position=position,force="player"}
    ...
    for _,part in ipairs(case.ingredients) do
      if part.type=="item" then
        ...
        local pump=surface.create_entity{name="factorio-test-intermediate-pump",position={x,machine.position.y-4.5},direction=defines.direction.south,force="player"}
        local buffer=surface.create_entity{name="factorio-test-intermediate-buffer",position={x,machine.position.y-6},force="player"}
...
script.on_nth_tick(960,function(event)
  ...
  helpers.write_file("factorio-tests/intermediate-recipes.json",helpers.table_to_json({
    schema=1,case="intermediate-recipes",status="pass",factorio_version=script.active_mods.base,
