...
-- place: 157 independent executors at crafting speed 60
...
local cases=require("__nullius-star__/scenarios/equipment-recipes/fixture")
...
local renamed={
  ["nullius-repair-pack"]="repair-pack",
}
local assertions=0
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("equipment-test",{width=128,height=576,autoplace_controls={}})
  ...
  for i,case in ipairs(cases) do
    ...
    local other="factorio-test-equipment-rejected"
    local position={((i-1)%5)*16-32,math.floor((i-1)/5)*16-240}
    local wrong=surface.create_entity{name="factorio-test-equipment-"..other,position=position,force="player"}
    wrong.set_recipe(name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-equipment-"..case.category,position=position,force="player"}
    ...
    for _,part in ipairs(case.ingredients) do
      if part.type=="item" then
        ...
        local buffer=surface.create_entity{name="factorio-test-equipment-buffer",position={x,machine.position.y-6},force="player"}
...
script.on_nth_tick(960,function(event)
  ...
  helpers.write_file("factorio-tests/equipment-recipes.json",helpers.table_to_json({
    schema=1,case="equipment-recipes",status="pass",factorio_version=script.active_mods.base,
