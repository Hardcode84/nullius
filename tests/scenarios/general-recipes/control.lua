...
-- place: 66 independent executors at crafting speed 60
...
local cases=require("__nullius-star__/scenarios/general-recipes/fixture")
...
local renamed={
  ["nullius-inserter-1"]="inserter",
  ["nullius-inserter-2"]="bob-turbo-inserter",
  ["nullius-inserter-3"]="bulk-inserter",
  ["nullius-inserter-4"]="bob-express-bulk-inserter",
  ["nullius-conveyor-belt-1"]="transport-belt",
  ["nullius-conveyor-belt-2"]="fast-transport-belt",
  ["nullius-conveyor-belt-3"]="express-transport-belt",
  ["nullius-conveyor-belt-4"]="bob-ultimate-transport-belt",
  ["nullius-underground-belt-1"]="underground-belt",
  ["nullius-underground-belt-2"]="fast-underground-belt",
  ["nullius-underground-belt-3"]="express-underground-belt",
  ["nullius-underground-belt-4"]="bob-ultimate-underground-belt",
  ["nullius-splitter-1"]="splitter",
  ["nullius-splitter-2"]="fast-splitter",
  ["nullius-splitter-3"]="express-splitter",
  ["nullius-splitter-4"]="bob-ultimate-splitter",
  ["nullius-power-switch"]="power-switch",
  ["nullius-rail"]="rail",
  ["nullius-train-stop"]="train-stop",
  ["nullius-rail-signal"]="rail-signal",
  ["nullius-chain-signal"]="rail-chain-signal",
  ["nullius-barrel-1"]="barrel",
  ["nullius-stone-block"]="stone-brick",
  ["nullius-wall"]="stone-wall",
  ["nullius-gate"]="gate",
}
local assertions=0
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("general-test",{width=128,height=288,autoplace_controls={}})
  ...
  for i,case in ipairs(cases) do
    ...
    local wrong=surface.create_entity{name="factorio-test-general-"..other,position=position,force="player"}
    wrong.set_recipe(name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-general-"..case.category,position=position,force="player"}
    ...
    for _,part in ipairs(case.ingredients) do
      if part.type=="item" then
        ...
        local buffer=surface.create_entity{name="factorio-test-general-buffer",position={x,machine.position.y-6},force="player"}
...
script.on_nth_tick(960,function(event)
  ...
  helpers.write_file("factorio-tests/general-recipes.json",helpers.table_to_json({
    schema=1,case="general-recipes",status="pass",factorio_version=script.active_mods.base,
