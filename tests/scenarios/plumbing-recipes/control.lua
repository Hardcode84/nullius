...
-- place: 113 independent executors at crafting speed 60
...
local cases=require("__nullius-star__/scenarios/plumbing-recipes/fixture")
...
local renamed={
  ["nullius-iron-pipe"]="pipe",
  ["nullius-underground-pipe-1"]="pipe-to-ground",
  ["nullius-steel-pipe"]="nullius-pipe-2",
  ["nullius-medium-tank-1"]="storage-tank",
  ["nullius-pump-3"]="pump",
}
local assertions=0
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("plumbing-test",{width=128,height=448,autoplace_controls={}})
  ...
  for i,case in ipairs(cases) do
    ...
    local other="factorio-test-plumbing-rejected"
    local position={((i-1)%5)*16-32,math.floor((i-1)/5)*16-176}
    local wrong=surface.create_entity{name="factorio-test-plumbing-"..other,position=position,force="player"}
    wrong.set_recipe(name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-plumbing-"..case.category,position=position,force="player"}
    ...
    for _,part in ipairs(case.ingredients) do
      if part.type=="item" then
        ...
        local buffer=surface.create_entity{name="factorio-test-plumbing-buffer",position={x,machine.position.y-6},force="player"}
...
script.on_nth_tick(960,function(event)
  ...
  helpers.write_file("factorio-tests/plumbing-recipes.json",helpers.table_to_json({
    schema=1,case="plumbing-recipes",status="pass",factorio_version=script.active_mods.base,
