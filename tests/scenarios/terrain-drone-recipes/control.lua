...
-- place: 15 independent executors at crafting speed 60
...
local cases=require("__nullius-star__/scenarios/terrain-drone-recipes/fixture")
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("terrain-drone-test",{width=128,height=96,autoplace_controls={}})
  ...
  for i,case in ipairs(cases) do
    ...
    if script.active_mods["factorio-test-support"] then
      local tech=game.forces.player.technologies[case.technology]
      local unlock=false
      for _,effect in ipairs(tech.prototype.effects) do
        if effect.type=="unlock-recipe" and effect.recipe==case.name then unlock=true end
      end
      check(unlock,case.name.." research unlock")
      tech.researched=true
      check(game.forces.player.recipes[case.name].enabled,case.name.." unlocked")
    else
    ...
    local wrong=surface.create_entity{name="factorio-test-terrain-"..other,position=position,force="player"}
    wrong.set_recipe(case.name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-terrain-"..case.category,position=position,force="player"}
...
script.on_nth_tick(600,function(event)
  ...
  helpers.write_file("factorio-tests/terrain-drone-recipes.json",helpers.table_to_json({
    schema=1,case="terrain-drone-recipes",status="pass",factorio_version=script.active_mods.base,
