...
-- place: nine independent executors at crafting speed 60
...
local cases=require("__nullius-star__/scenarios/alignment-recipes/fixture")
local technologies=require("__nullius-star__/scenarios/alignment-recipes/technologies")
...
script.on_nth_tick(1,function()
  ...
  local surface=game.create_surface("alignment-test",{width=128,height=96,autoplace_controls={}})
  surface.request_to_generate_chunks({0,0},3)
  surface.force_generate_chunk_requests()
  for _,entity in pairs(surface.find_entities()) do entity.destroy() end
  storage.machines={}
  for i,case in ipairs(technologies) do
    local tech=game.forces.player.technologies["nullius-alignment-"..i]
    tech.enabled=true
    tech.researched=true
    for _,name in ipairs(case.unlocks) do
      check(game.forces.player.recipes["nullius-"..name].enabled,"research unlock "..name)
    end
  end
  for i,case in ipairs(cases) do
    ...
    check(prototypes.item["nullius-box-"..case.name:sub(9)]==nil,case.name.." no boxed alignment item")
    local other=case.category=="tiny-crafting" and "small-crafting" or "tiny-crafting"
    local position={((i-1)%8)*8-32,math.floor((i-1)/8)*8-20}
    local wrong=surface.create_entity{name="factorio-test-alignment-"..other,position=position,force="player"}
    wrong.set_recipe(case.name)
    check(wrong.get_recipe()==nil,case.name.." rejects other category")
    wrong.destroy()
    local machine=surface.create_entity{name="factorio-test-alignment-"..case.category,position=position,force="player"}
...
script.on_nth_tick(600,function(event)
  ...
  helpers.write_file("factorio-tests/alignment-recipes.json",helpers.table_to_json({
    schema=1,case="alignment-recipes",status="pass",factorio_version=script.active_mods.base,
