...
local CASE = destination .. "-shared-body"
local planet_name = "nullius-" .. destination
local other_destination = destination == "vulcanus" and "fulgora" or "vulcanus"
local function call(method, ...)
  if method == "quick_start" or method == "activate" or method == "snapshot" then
    local argument = ...
    return remote.call("nullius-test-bodies", method, argument, destination)
  end
...
local function stock()
  local surface = game.planets[planet_name].surface
...
script.on_nth_tick(30, function()
  ...
  if stage == "first-join" and a and a.connected and storage.joins[a.name] then
    ...
    research.complete_with_prerequisites(a.force.technologies[destination == "vulcanus" and "nullius-pneumatic-technology" or "nullius-probe-fulgora"])
  ...
  elseif stage == "respawn" and storage.respawned == b.index then
    ...
    remote.call("nullius-test-bodies", "activate", a.force, other_destination)
    local other = remote.call("nullius-test-bodies", "snapshot", a.index, other_destination).body
    check(other and other.valid and other ~= storage.probe, "planets share a probe body")
    check(other.surface.name == "nullius-" .. other_destination, "other probe has wrong planet")
    storage.other_planet_unit = other.unit_number
  ...
  elseif stage == "reload" and a.connected and b.connected and
    ...
    stock()
    local other = remote.call("nullius-test-bodies", "snapshot", a.index, other_destination).body
    check(other and other.valid and other.unit_number == storage.other_planet_unit,
      "reload lost the other planet's probe")
