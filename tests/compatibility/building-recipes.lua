local cases=require("scenarios/building-recipes/fixture")
...
for _,name in ipairs({"boxed-beacon","boxed-circuit","boxed-fluid-energy","boxed-heat-energy","boxed-power-pole","boxed-rail","boxed-renewable"}) do
  if not data.raw["item-subgroup"][name] then data:extend({{type="item-subgroup",name=name,group="production",order="z"}}) end
end
require("building-source")
