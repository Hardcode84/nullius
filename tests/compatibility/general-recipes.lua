local cases=require("scenarios/general-recipes/fixture")
...
for _,name in ipairs({"boxed-belt","boxed-canister","boxed-circuit","boxed-inserter","boxed-rail","boxed-terrain","canisters","canister-emptying"}) do
  if not data.raw["item-subgroup"][name] then data:extend({{type="item-subgroup",name=name,group="production",order="z"}}) end
end
require("general-source")
