...
data:extend({{type="recipe-category",name="factorio-test-vulcanus-entity-rejected"}})
local variants={["factorio-test-vulcanus-entity-rejected"]={category="factorio-test-vulcanus-entity-rejected",inputs=0,outputs=0}}
...
for name,variant in pairs(variants) do
  ...
  machine.name="factorio-test-vulcanus-entity-"..name
  machine.minable=nil
...
buffer.name="factorio-test-vulcanus-entity-buffer"
buffer.fluid_box.volume=10000
data:extend({buffer})

local pump=table.deepcopy(data.raw.pump["factorio-test-void-pump"])
pump.name="factorio-test-vulcanus-entity-pump"
