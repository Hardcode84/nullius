...
remote.add_interface("release-upgrade", {
  seed = function()
    ...
    local lost = game.create_force("lost-probe")
    probe.on_probe_researched("nullius-probe-vulcanus", lost)
    probe.get_landing(lost).android.destroy()
  end,
  verify = function()
    assert(script.active_mods["nullius-star"] == candidate_version, "candidate was not loaded")
    ...
    local lost = game.forces["lost-probe"]
    probe.on_probe_researched("nullius-probe-vulcanus", lost)
    assert(not probe.get_landing(lost).android.valid, "upgrade recreated the destroyed probe")
    assert(body.surface.count_entities_filtered{name = "nullius-landing-main", force = lost} == 1,
      "upgrade duplicated destroyed-probe supplies")
    probe.on_probe_researched("nullius-probe-fulgora", force)
    local fulgora = probe.get_landing(force, "nullius-fulgora")
    assert(fulgora and fulgora.android.valid and fulgora.android.surface.name == "nullius-fulgora",
      "upgraded force cannot activate Fulgora")
    assert(probe.get_landing(force).android == body, "Fulgora overwrote the upgraded Vulcanus body")
    helpers.write_file("upgrade-result.json", helpers.table_to_json{
      status = "pass", from = "0.0.4", to = script.active_mods["nullius-star"],
