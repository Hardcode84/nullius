...
def analyze_science_scale(data, report, spec, entrance):
    """Research budgets and gross pack inputs; no construction-time estimate."""
    budgets = {name: research_cost(data, roots, entrance)
    ...
    for name, budget in budgets.items():
        if budget["unquantified"]:
            raise TestFailure("science analysis requires exact research costs")
        packs = {}
        for pack in spec["packs"]:
            count = budget["packs"].get(pack, 0)
            drivers = sorted(({"technology": t["name"], "packs": t["packs"].get(pack, 0)}
                              for t in budget["technologies"] if t["packs"].get(pack, 0)),
                             key=lambda t: (-t["packs"], t["technology"]))
            packs[pack] = {"total": count,
                "supply_hours": {str(rate): count / rate / 60 for rate in spec["rates"]},
                "required_rate_per_minute": {str(hours): count / hours / 60 for hours in spec["hours"]},
                "top_technologies": drivers[:10]}
        rows.append({"boundary": name, "all_packs": budget["packs"], "packs": packs,
                     "technology_count": budget["technology_count"]})
    lines = []
    for stage in report["stages"]:
        for plan in stage["plans"]:
            if plan["flow"]["status"] != "optimal":
            ...
            for pack in spec["packs"]:
                ...
                for recipe in plan["flow"]["recipes"]:
                    outputs = {p: recipe["flows"].get(p, 0) + recipe["inputs"].get(p, 0) for p in products}
                    outputs = {p: q for p, q in outputs.items() if q > 0}
                    if not outputs:
                        continue
                    product, output = next(iter(sorted(outputs.items())))
                    cycles = recipe["cycles_per_minute"]
                    producers.append({"recipe": recipe["recipe"], "machine": recipe["machine"],
                        "count": math.ceil(cycles * recipe["seconds"] / 60 - 1e-9),
                        "product": product, "output_per_minute": output * cycles,
                        "inputs_per_minute": {p: q * cycles for p, q in recipe["inputs"].items()}})
                if producers:
                    lines.append({"stage": stage["name"], "rate": plan["rate_per_minute"], "pack": pack,
                        "producers": producers, "factory_machines": plan["factory"]["process_machines"],
...
def write_science_scale(report, path):
    analysis = report["science_analysis"]
    lines = ["# Vulcanus geology and climatology scale", "",
    ...
    for budget in analysis["budgets"]:
        for pack, row in budget["packs"].items():
            lines.append(f"| {budget['boundary']} | {pack} | {row['total']:,.0f} | "
                         f"{row['supply_hours']['60']:.2f} | {row['supply_hours']['120']:.2f} | "
                         f"{row['required_rate_per_minute']['8']:.2f} | {row['required_rate_per_minute']['4']:.2f} |")
    lines += ["", "## Production capacity", "",
    ...
    for row in analysis["production_lines"]:
        stations = "; ".join(f"{p['recipe']}: {p['count']}" for p in row["producers"])
        lines.append(f"| {row['stage']} | {row['rate']:g} | {row['pack']} | {stations} | "
                     f"{row['factory_machines']} | {row['fuel_per_minute']:,.0f} | {row['heat_mw']:.2f} | {row['construction_status']} |")
    lines += ["", "## Largest research costs", ""]
    for budget in analysis["budgets"]:
        if "@" in budget["boundary"] and not budget["boundary"].endswith("@120"):
            continue
        for pack, row in budget["packs"].items():
            lines += [f"### {budget['boundary']}: {pack}", "", "| Technology | Packs |", "|---|---:|"]
            lines += [f"| {t['technology']} | {t['packs']:,.0f} |" for t in row["top_technologies"]]
            lines.append("")
    lines += ["## Model boundary", ""]
    lines += [f"- **{k}:** {v}" for k, v in report["assumptions"].items()]
    path.write_text("\n".join(lines) + "\n")
...
def write_comparison(report, path):
    ...
    lines = ["# " + spec["title"], "",
             ...
             "| Route | Product | Units/min | Flow | Construction | Machines | Active machine equivalents | "
    ...
    for stage in report["stages"]:
        for plan in stage["plans"]:
            ...
            demands = plan["demands_per_minute"]
            if "product" not in spec and len(demands) != 1:
                raise TestFailure("comparison needs one target per stage or an explicit product")
            product = spec.get("product", next(iter(demands)))
            ...
            lines.append(f"| {stage['name']} | {product} | {plan['demands_per_minute'][product]:g} | optimal | "
...
def main():
    ...
    if args.read_plan:
        ...
        try:
            ...
            if "science_analysis" in config:
                report["science_analysis"] = analyze_science_scale(data, report, config["science_analysis"], config["entrance_technologies"])
