#!/usr/bin/env python3
"""Check the optional textplates recipe on Factorio 2.0 and 2.1 with declared external prototypes."""
import argparse
import json
...
def run(factorio, mode):
    ...
    work = Path(tempfile.mkdtemp(prefix="textplate-compat-"))
    ...
    (mod / "info.json").write_text(json.dumps({
        ...
        "title": "Text Plates recipe fixture", "author": "tests", "dependencies": ["base", "? textplates"],
    }))
    external = work / "mods/textplates"
    external.mkdir()
    (external / "info.json").write_text(json.dumps({
        "name": "textplates", "version": "0.0.1", "factorio_version": version,
        "title": "External item fixture", "author": "tests", "dependencies": ["base"],
    }))
    names = ["textplates", "base", "space-age", "quality", "elevated-rails", "nullius-star"]
    if version == "2.1":
        names.append("recycler")
    (work / "mods/mod-list.json").write_text(json.dumps({"mods": [{"name": name, "enabled": name in ({"base", "nullius-star", "textplates"} if mode != "no-mod" else {"base", "nullius-star"})} for name in names]}))
    source = (ROOT / "nullius-star/prototypes/override_mod.lua").read_text()
    start = source.index('if mods["textplates"] then')
    end = source.index('if mods["rso-mod"] then', start)
    ...
    for target, source_path in (
        ...
        ("fixture.lua", "tests/compatibility/textplate-recipes.lua"),
        ("executor.lua", "tests/factorio-test-support/textplate-recipes.lua"),
        ("scenarios/textplate-recipes", "tests/scenarios/textplate-recipes"),
    ):
    ...
    deadline = json.loads((ROOT / "tests/scenarios/textplate-recipes/test.json").read_text())["until_tick"]
    config = prepare_config(work, factorio)
    common = [str(factorio), "--config", str(config), "--mod-directory", str(work / "mods"), "--disable-audio"]
    for label, options in (
        ("dump", ["--dump-data"]),
        ("compile", ["--scenario2map", "nullius-star/textplate-recipes"]),
        ("run", ["--load-game", str(work / "saves/nullius-star/textplate-recipes.zip"), "--until-tick", str(deadline)]),
    ):
        result = run_factorio(common + options, work / f"{label}.log", 180)
        if result.returncode:
            raise TestFailure(f"{label} failed; artifacts: {work}\n{result.stdout[-5000:]}")
    result = json.loads((work / "script-output/factorio-tests/textplate-recipes.json").read_text())
