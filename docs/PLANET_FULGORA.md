...
|---|---|---|
| Pole collectors | Linked pole and attractor, pole targeting plus scripted energy injection, or hidden attractor attached to each pole | Verify energy delivery; pole and attractor are separate prototype types |
...
| Strike effects | `LightningPrototype.energy`, `damage`, `time_to_damage`, `strike_effect`, `attractor_hit_effect` | Test ordinary and attracted strikes separately |
| No destruction | Prefer zero lightning damage; alternatives are entity exemptions or restoring damage in an event | Verify that all relevant strike paths preserve buildings |
